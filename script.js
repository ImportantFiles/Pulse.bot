...
function calculateGrowthFromAccountInfo(ocrData) {
  ...
  if (ocrData.balance === null || netDeposit === null || netDeposit === 0) {
    console.warn("Growth not calculated: Balance or Net Deposit is missing, or Net Deposit is zero.", {
  ...
  const growth = roundToTwo(((ocrData.balance - netDeposit) / netDeposit) * 100);
...
function logOcrCalculation(ocrData) {
  ...
  console.log("Net Deposit:", ocrData?.netDeposit ?? null);
